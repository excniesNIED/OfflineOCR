...
"""
自动下载并设置OCR模型的脚本
该脚本会自动下载PaddleOCR模型并将其复制到项目目录的models文件夹中（适配旧模型结构）
"""
...
def copy_models():
    """
    将下载的模型复制到项目目录的models文件夹中（适配旧模型结构）
    """
    try:
        # 首先尝试查找 .paddleocr 目录
...
def main():
    print("=" * 60)
    print("PaddleOCR 模型自动设置工具")
    print("=" * 60)
    print("该工具将自动下载PaddleOCR模型并复制到项目目录中")
    print("此工具将模型组织为旧结构以确保兼容性")
    print()
    
    # 检查是否已经存在models目录
    if os.path.exists(os.path.join(os.getcwd(), "models")):
        response = input("检测到已存在models目录，是否重新下载并覆盖？(y/N): ")
